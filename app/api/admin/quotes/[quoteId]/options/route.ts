...
export async function POST(request: Request, { params }: { params: Promise<{ quoteId: string }> }) {
  ...
  const { data: quote, error: quoteError } = await supabase
    .from('quote_requests')
    .select('id, has_power, has_water, distance_miles, base_price, travel_fee, utility_fee, after_hours_fee, cleaning_fee, damage_waiver_fee, rush_booking_fee, subtotal, discount_amount, total_price, deposit_amount, final_balance, calculated_breakdown, needs_manual_distance_review')
    .eq('id', quoteId)
    .single();
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      ...pricing,
      calculated_breakdown: quote.calculated_breakdown ?? null,
