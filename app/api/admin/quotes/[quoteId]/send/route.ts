...
export async function POST(_request: Request, { params }: { params: Promise<{ quoteId: string }> }) {
  ...
  const { data: quote, error } = await supabase
    .from('quote_requests')
    .select('id,quote_number,customer_name,customer_email,event_date,event_type,total,deposit_amount,status')
    .eq('id', quoteId)
    .single();

  if (error || !quote) return NextResponse.json({ ok: false, message: 'Quote not found' }, { status: 404 });
  if (!canSendQuote((quote.status ?? 'new') as QuoteStatus)) return NextResponse.json({ ok: false, message: 'Quote cannot be sent in current status.' }, { status: 400 });
  ...
  const sendResult = await sendEmail({ to: quote.customer_email, subject: tmpl.subject, html: tmpl.html });
  ...
  await supabase.from('quote_requests').update({ status: 'sent_to_customer', quote_sent_at: new Date().toISOString(), approval_token: tokenHash, updated_at: new Date().toISOString() }).eq('id', quote.id);
